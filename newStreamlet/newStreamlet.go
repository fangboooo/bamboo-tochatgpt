...
type newConsensus struct {
	...
	buferrBlockNum         map[crypto.Identifier]int
...
func NewStreamlet(
	...
	sl.buferrBlockNum = make(map[crypto.Identifier]int)
...
func (sl *newConsensus) ProcessBlock(block *blockchain.Block) error {
	...
	if !exists {
		...
		sl.Broadcast(block)
		sl.buferrBlockNum[block.ID]++
		if block.Proposer != sl.ID() {
			return nil
		}
	} else if sl.buferrBlockNum[block.ID] != (config.GetConfig().N()-1)/3 {
		sl.buferrBlockNum[block.ID]++
		return nil
	}
